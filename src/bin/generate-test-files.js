...
const generatedFileDirectory =
  serverFile === 'yes' ? join(__dirname, '..', 'src', 'server') : __dirname;
...
if (commitAndPush === 'yes') {
  writeFile(filePath, stuff, null, () =>
    exec(
      `git add ${filePath}; git commit --no-verify -m '${lineCount}${
        excludedFile && ' mock'
      }'; git push`,
      (error, stdout, stderr) =>
        console.log(
